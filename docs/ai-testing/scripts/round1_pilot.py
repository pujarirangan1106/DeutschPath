"""
Pilot A/B test: Gemini vs OpenAI-compatible (SAIA qwen3-30b-a3b-instruct-2507)
Judged blind by qwen3.6-27b (a third, neutral SAIA model).

Scope (pilot, per user request): 1 level (A2), 2 tasks per section, all 6 sections.
"""
import asyncio
import json
import os
...
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/../../../../../../../home/rangan-ubuntu/Deutsch/DeutschPath/backend")
...
os.environ["MODEL"] = "mistral-medium-3.5-128b"
...
# ── 2. Grammar exercises ─────────────────────────────────────────────────────
TASKS.append({
    "section": "grammar_exercises", "level": "A2", "task_id": "gramex_1",
    "fn": "generate_grammar_exercises",
    "kwargs": dict(rule_name="Perfekt with haben/sein", pattern="haben/sein + Partizip II",
                   ...
                   example_de="Ich habe gegessen.", user_level="A2",
                   secondary_lang_name="English", secondary_lang_code="en"),
})
TASKS.append({
    "section": "grammar_exercises", "level": "A2", "task_id": "gramex_2",
    "fn": "generate_grammar_exercises",
    "kwargs": dict(rule_name="Modalverben (können, müssen, wollen)", pattern="Modal + Infinitiv am Satzende",
                   ...
                   example_de="Ich muss heute arbeiten.", user_level="A2",
                   secondary_lang_name="English", secondary_lang_code="en"),
})

# ── 3. Grammar explanation ───────────────────────────────────────────────────
TASKS.append({
    "section": "grammar_explanation", "level": "A2", "task_id": "gramexp_1",
    "fn": "analyze_grammar",
...
TASKS.append({
    "section": "grammar_explanation", "level": "B1", "task_id": "gramexp_2",
    "fn": "analyze_grammar",
...
# ── 4. Writing feedback ──────────────────────────────────────────────────────
TASKS.append({
    "section": "writing_feedback", "level": "A2", "task_id": "writing_1",
    "fn": "analyze_writing",
    "kwargs": dict(
        user_text=(
...
TASKS.append({
    "section": "writing_feedback", "level": "A2", "task_id": "writing_2",
    "fn": "analyze_writing",
    "kwargs": dict(
        user_text=(
            "Meine Stadt heisst München. Es ist eine grosse Stadt in Deutschland. Ich wohne hier seit drei Jahre. "
...
# ── 5. Translation ───────────────────────────────────────────────────────────
TASKS.append({
    "section": "translation", "level": "A2", "task_id": "trans_1",
    "fn": "translate_text",
    "kwargs": dict(text="Ich gehe morgen mit meinen Freunden ins Kino.", target_language="English"),
})
TASKS.append({
    "section": "translation", "level": "B1", "task_id": "trans_2",
    "fn": "translate_text",
...
# ── 6. Grammar chat practice ─────────────────────────────────────────────────
TASKS.append({
    "section": "grammar_chat", "level": "A2", "task_id": "chat_1",
    "fn": "chat_grammar_practice",
...
TASKS.append({
    "section": "grammar_chat", "level": "A2", "task_id": "chat_2",
    "fn": "chat_grammar_practice",
    "kwargs": dict(rule_name="Perfekt with sein", rule_explanation="Movement/change-of-state verbs form the Perfekt with sein instead of haben.",
...
async def run_provider(fn_name, kwargs, provider):
...
def judge_prompt(section, task_id, kwargs, resp_a_text, resp_b_text):
...
async def main():
    ...
    for task in TASKS:
        ...
        openai_res = await run_provider(task["fn"], task["kwargs"], "openai")
        ...
        # randomize A/B mapping to reduce position bias
        flip = random.random() < 0.5
        if flip:
            resp_a_text, resp_b_text = gemini_text, openai_text
            a_provider, b_provider = "gemini", "openai"
        else:
            resp_a_text, resp_b_text = openai_text, gemini_text
            a_provider, b_provider = "openai", "gemini"

        judge_raw = None
        judge_parsed = None
        if gemini_res["ok"] and openai_res["ok"]:
            try:
                jp = judge_prompt(task["section"], task["task_id"], task["kwargs"], resp_a_text, resp_b_text)
                judge_raw = call_judge(jp)
        ...
        print(f"  gemini: ok={gemini_res['ok']} {gemini_res['latency_s']}s | openai: ok={openai_res['ok']} {openai_res['latency_s']}s | judge preferred={((judge_parsed or {}).get('preferred'))}", file=sys.stderr)

    out_path = "/tmp/claude-1000/-home-rangan-ubuntu-Deutsch-DeutschPath/a76b82c9-1485-4390-92e7-0788ff707c09/scratchpad/ab_pilot_results_mistral.json"
